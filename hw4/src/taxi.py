...
class Agent():
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        self.qtable[state, action] = value - self.learning_rate*(value - (reward + self.gamma*next_value))
        # End your code

        # You can add some conditions to decide when to save your table
        if done and self.score_rec > self.max_score:
...
def train(env):
    ...
    for ep in tqdm(range(episode)):
        ...
        while True:
            ...
            count += reward
            if done and ep > 2000:
                training_agent.score_rec = 0.5*count + 0.5*training_agent.score_rec
