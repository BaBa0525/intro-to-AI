...
class Agent():
    ...
    def learn(self):
        ...
        rewards = torch.tensor(rewards, dtype=torch.float).view(self.batch_size, -1)
        next_observations = torch.tensor(np.array(next_observations), dtype=torch.float)
        done = torch.tensor(done, dtype=torch.float)
        
        q_eval = self.evaluate_net.forward(observations).gather(1, actions)
        #q_eval = self.evaluate_net(observations).gather(1, actions)
        q_next = self.target_net.forward(next_observations).detach()
        q_target = rewards + self.gamma * ((1-done) * torch.max(q_next, dim=1)[0]).view(self.batch_size, -1)
        ...
        if self.count % 100 == 0 and self.score_rec > self.max_score:
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            if np.random.uniform(0, 1) < self.epsilon:
                ...
                Q = self.evaluate_net.forward(
                torch.FloatTensor(state)).squeeze(0).detach()
                action = int(torch.argmax(Q).numpy())

                # actions_value = self.evaluate_net.forward(state)
                # action = torch.max(actions_value, 1)[1].data.numpy()
                # action = action[0]

            # End your code
        #print(self.evaluate_net(state))
        return action
...
def train(env):
    ...
    for _ in tqdm(range(episode)):
        ...
        while True:
            ...
            if agent.count >= 1000:
                if done:
                    agent.score_rec = 0.9*count + 0.1*agent.score_rec
                agent.learn()
            if done:
                rewards.append(count)
                break
            state = next_state
        if agent.max_score > max_value:
            max_value = agent.max_score
