...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        """
        # Begin your code
        arr = np.linspace(lower_bound, upper_bound, num_bins+1)
        return arr[1 : num_bins]
        # End your code
    ...
    def discretize_observation(self, observation):
        ...
        """
        # Begin your code
        cartp, cartv, polang, tipv = observation
        cartp = self.discretize_value(cartp, self.bins[0])
        cartv = self.discretize_value(cartv, self.bins[1])
        polang = self.discretize_value(polang, self.bins[2])
        tipv = self.discretize_value(tipv, self.bins[3])

        return [cartp, cartv, polang, tipv]
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        """
        # Begin your code
        value = self.qtable[state[0], state[1], state[2], state[3], action]
        next_value = np.max(self.qtable[tuple(next_state)])
        
        #print(value - self.learning_rate*(value - (reward + self.gamma*next_value)))
        self.qtable[state[0], state[1], state[2], state[3], action] = value - self.learning_rate*(value - (reward + self.gamma*next_value))
        # End your code

        # You can add some conditions to decide when to save your table

        if done:
            self.score_rec =  0.9*self.score + 0.1*self.score_rec
            if self.score_rec > self.max_score:
                self.max_score = self.score_rec
...
def train(env):
    ...
    for ep in tqdm(range(episode)):
        ...
        training_agent.score = 0
        training_agent.max_score = max_value
        while True:
            count += 1
            training_agent.score += 1
        ...
        if training_agent.max_score > max_value:
            max_value = training_agent.max_score
